...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)    echo "usage: $0 [--version vX.Y.Z] [--api-host URL] [--node-id N] [--secret-key KEY] [--system]"; exit 0 ;;
    *) die "未知参数: $1" ;;
  esac
done
...
# --- binary ---
mkdir -p "$INSTALL_DIR"
# 装到临时文件再原子替换:不能直接覆盖正在运行的二进制(Text file busy)。
if [[ -n "${EXNODE_BIN:-}" ]]; then
  say "安装本地二进制 $EXNODE_BIN"
  install -m 0755 "$EXNODE_BIN" "${BIN}.new"
else
  if [[ -z "$VERSION" ]]; then
    say "查询最新版本"
  ...
  curl -fSL "https://github.com/${REPO}/releases/download/${VERSION}/exnode-linux-${arch}" -o "${BIN}.new" || { rm -f "${BIN}.new"; die "下载失败"; }
  chmod +x "${BIN}.new"
fi
mv -f "${BIN}.new" "$BIN"
...
case "${1:-}" in
  ...
  update)
    ...
    curl -fSL "https://github.com/${REPO}/releases/download/${v}/exnode-linux-$(arch)" -o "${BIN}.new" || { echo -e "${r}下载失败${p}"; rm -f "${BIN}.new"; exit 1; }
    chmod +x "${BIN}.new"; mv -f "${BIN}.new" "$BIN"
    systemctl restart exnode; echo -e "${g}已更新并重启 ${v}${p}" ;;
  uninstall)
  ...
  *) echo "usage: exnode {start|stop|restart|status|log|update|uninstall|config}" ;;
esac
EXNODE_CLI
